...
class FriendListComponent extends React.Component {
        ...
        follow(client, this.props.root, [{rel : 'cf_profile'}])
            .then(cf_profilePromise => {
                ...
                return client({
                    ...
                    console.log('friend list promise: ', friendListPromise.entity._embedded);
                    this.setState({
                        friends : friendListPromise.entity._embedded.friend
                    });
    ...
    togglePopupVisibility(id) {
        ...
        if(friendDivElement) {
            ...
            popupDivElement.style.top= rect.top+'px';
            popupDivElement.style.right = (rect.width + 20) + 'px';

            this.state.friends.forEach((friend) => {
                if(friend.friendProfile.name === friendDivElement.id)
                    this.setState({selectedFriend: friend.friendProfile});
            });
    ...
    render() {
        let orderedList = this.state.friends.map( friend =>
            <div key={friend.friendProfile.name} ref={friend.friendProfile.name} id={friend.friendProfile.name} className='row ui-friend-div'
                 onMouseEnter={() => this.togglePopupVisibility(friend.friendProfile.name)}
                 onMouseLeave={this.togglePopupVisibility}>

                <div className='col-2'>
                        <img src={'data:image/jpeg;base64, '+friend.friendProfile.profilePicture} alt='no' className='ui-friend-profile-picture'/>
                </div>
                <div className='col-8'>
                    <label className='ui-friend-text' >
                        {friend.friendProfile.firstName} {friend.friendProfile.lastName}
                    </label>
                </div>
        ...
        let popup = <div className='ui-friend-menu popup-text' ref='popup' style={this.state.popupVisibility}>
            <div className='row'>
                <div className='col-4 col-t-4'>
                    <img src={'data:image/jpeg;base64, '+this.state.selectedFriend.profilePicture} alt='no' className='ui-friend-menu-profile-picture'/>
                </div>
